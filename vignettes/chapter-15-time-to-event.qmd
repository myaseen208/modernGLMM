...
- **Cox PH model**: $h(t \mid \mathbf{x}) = h_0(t) \exp(\mathbf{x}^\top\pmb{\beta})$
- **Frailty model**: adds a random effect $u_i$ on the log hazard scale

$$h(t \mid \mathbf{x}_i, u_i) = h_0(t) \exp(\mathbf{x}_i^\top\pmb{\beta} + u_i), \quad u_i \sim \mathcal{N}(0, \sigma^2)$$
