...
* Applied the 12-step MISMATCH diagnostic to all 18 remaining MISMATCH rows;
  all 18 confirmed irreducible with documented root causes.
* Retained all 5 functions present in the 2nd edition that have MISMATCH rows
  (`Exam8.7`, `Exam9.4`, `Exam10.2`, `Exam10.4`, `Exam11.3`).
  Functions are not removed on the basis of MISMATCH status.
* Removed only functions confirmed absent from the 2nd edition PDF via direct
  chapter reading (`Exam2.B.1`–`Exam2.B.7`, `Exam8.6`, `Exam11.4`,
  `Exam12.1`, `Exam12.2`, `Exam14.1`, `Exam14.2`,
  `Exam17.1`, `Exam17.2`, `Exam18.1`, `Exam18.2`, `Exam21.2`).
  Their datasets are retained for vignette use.
* Fixed `Exam9.4` example formula: corrected `y ~ a + b %in% a` (nested) to
  `y ~ a * b` (crossed) to match the book's Type III F test structure and the
  verification script.
