# Chapter 5: GLMM Estimation

## 1 Overview

Chapter 5 of Stroup, Ptukhina & Garai (2024) presents the **estimation
framework for GLMMs** — the mathematical machinery behind
likelihood-based fitting of mixed models. This chapter is entirely
theoretical and contains no applied examples or datasets. Core topics
are:
...
\\f(y;\\\theta, \phi) = \exp\\\left\\\frac{y\theta -
b(\theta)}{a(\phi)} + c(y, \phi)\right\\\\

where \\\theta\\ is the **natural (canonical) parameter**, \\b(\theta)\\
determines the mean via \\\mu = b'(\theta)\\, and \\\phi\\ is a
...
```
    ...
    Block variance  — ML: 1.9381  |  REML: 2.3909
    Residual variance— ML: 1.6294  |  REML: 1.6294

## 5 Newton-Raphson and Fisher Scoring
...
distribution is also Gaussian (i.e., the Gaussian LMM). For other GLMMs
it introduces \\O(n^{-1})\\ approximation error. Adaptive Gauss-Hermite
quadrature (AGHQ) achieves higher accuracy at greater computational
cost.
...
Press. Chapter 5: GLMM Estimation, pp. 146+.
