...
framework treats all unknowns — fixed effects \\\boldsymbol{\beta}\\,
random effects \\\mathbf{b}\\, and variance components
\\\boldsymbol{\theta}\\ — as random variables with prior distributions.
...
- `brms::brm()` fits Bayesian GLMMs with the same formula syntax as
  `lme4`.
