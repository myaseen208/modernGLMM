...
For a linear model \\\mathbf{y} = \mathbf{X}\pmb{\beta} +
\pmb{\varepsilon}\\ with \\\pmb{\varepsilon} \sim
\mathcal{N}(\mathbf{0}, \sigma^2 \mathbf{I})\\, the OLS estimator is:

\\\hat{\pmb{\beta}}\_{\text{OLS}} = (\mathbf{X}^\top \mathbf{X})^{-}
...
\\\hat{\pmb{\beta}}\_{\text{GLS}} = (\mathbf{X}^\top \mathbf{V}^{-1}
\mathbf{X})^{-} \mathbf{X}^\top \mathbf{V}^{-1} \mathbf{y}\\
...
An estimator \\\hat{\pmb{\ell}} = \mathbf{c}^\top \hat{\pmb{\beta}}\\ is
BLUE for \\\pmb{\ell} = \mathbf{c}^\top \pmb{\beta}\\ if:
...
A linear combination \\\mathbf{c}^\top \pmb{\beta}\\ is **estimable** if
and only if \\\mathbf{c}^\top\\ lies in the row space of \\\mathbf{X}\\,
i.e., \\\mathbf{c}^\top = \mathbf{t}^\top \mathbf{X}\\ for some
\\\mathbf{t}\\.
...
- If \\\mathbf{y} \sim \mathcal{N}(\pmb{\mu}, \sigma^2 \mathbf{I})\\ and
  \\\mathbf{A}\\ is idempotent of rank \\r\\, then \\\mathbf{y}^\top
  \mathbf{A} \mathbf{y} / \sigma^2 \sim \chi^2_r(\lambda)\\ with
  non-centrality \\\lambda = \pmb{\mu}^\top \mathbf{A} \pmb{\mu} /
  \sigma^2\\.
