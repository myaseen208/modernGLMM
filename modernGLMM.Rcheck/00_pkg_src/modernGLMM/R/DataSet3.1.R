#' @title Data for Example 3.1 and Example 3.2 from Generalized Linear Mixed Models: Modern Concepts, Methods and Applications by Stroup, Ptukhina, and Garai (2024, 2nd ed.)
#' @name   DataSet3.1
#' @docType data
#' @keywords datasets
#' @usage data(DataSet3.1)
#' @description DataSet3.1 is used for linear and generalized linear models
#' @format A \code{data.frame} with 20 rows and 5 variables.
#' @details
#'        \itemize{
#'        \item trt two treatment 0 and 1
#'        \item rep unit of observation or observation ID
#'        \item Y  is continuous & may be assumed Gaussian
#'        \item N  is the number of obs
#'        \item F  is the number of "successes"(N and F specify a binomial response)
#'        }
#' @author \enumerate{
#'          \item  Muhammad Yaseen (\email{myaseen208@@gmail.com})
#'          \item Adeela Munawar (\email{adeela.uaf@@gmail.com})
#'          }
#' @references \enumerate{
#' \item Stroup, W. W., Ptukhina, M., and Garai, S. (2024).\emph{Generalized linear mixed models: modern concepts, methods and applications}.
#'              CRC Press.
#'  }
#' @seealso
#'    \code{\link{Exam3.2}}
#' @examples
#' data(DataSet3.1)
NULL
